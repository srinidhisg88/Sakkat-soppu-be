...
exports.getDeliverySettings = async (req, res) => {
  try {
    ...
    const data = cfg || { enabled: true, deliveryFee: 0, freeDeliveryThreshold: 0 };
...
exports.updateDeliverySettings = async (req, res) => {
  try {
    const { enabled, deliveryFee, freeDeliveryThreshold } = req.body || {};
    const payload = {};
...
exports.getPublicDeliverySettings = async (req, res) => {
  try {
    ...
    const { enabled = true, deliveryFee = 0, freeDeliveryThreshold = 0 } = cfg;
    return res.status(200).json({ enabled, deliveryFee, freeDeliveryThreshold });
  } catch (e) {
