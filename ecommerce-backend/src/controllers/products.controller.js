...
exports.createProduct = async (req, res) => {
    ...
    const { name, category, categoryId, price, stock, description, isOrganic } = req.body;
        // Prefer one of litre, g, or pieces; litre takes highest priority
        let g = Number(req.body.g || 0) || 0;
        let pieces = Number(req.body.pieces || 0) || 0;
        let litre = Number(req.body.litre || 0) || 0;
        if (litre > 0) {
            // If litre is set, clear others
            g = 0;
            pieces = 0;
        } else if (g > 0 && pieces > 0) {
...
exports.updateProduct = async (req, res) => {
    try {
    const { name, category, categoryId, price, stock, description, isOrganic } = req.body;
    // Normalize litre/g/pieces: litre takes highest priority
    let g = Number(req.body.g || 0) || 0;
    let pieces = Number(req.body.pieces || 0) || 0;
    let litre = Number(req.body.litre || 0) || 0;
    if (litre > 0) {
